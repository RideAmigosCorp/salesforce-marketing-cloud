...
public class SwiftSfmcPlugin: NSObject, FlutterPlugin {
  ...
  public func setupSFMC(
    appId: String, accessToken: String, mid: String, sfmcURL: String, delayRegistration: Bool,
    analytics: Bool, result: @escaping FlutterResult
  ) {
  ...
  func setupMobilePush() {
    ...
    DispatchQueue.main.async {
      ...
      UNUserNotificationCenter.current().requestAuthorization(
        ...
        completionHandler: { (_ granted: Bool, _ error: Error?) -> Void in
          if error == nil {
            if granted == true {
